...
public:
    ...
    friend std::basic_ostream<char>& operator<<(std::basic_ostream<char>& out, const matrix & mat)
    {
        for (size_type i = 0; i < mat.rows_count; ++i)
        {
            for (size_type j = 0; j < mat.cols_count; ++j)
            {
                out << mat(i, j) << " ";
            }
            out << "\n";
    ...
    friend std::basic_istream<char>& operator >>(std::basic_istream<char> & in, matrix & mat)
    {
        std::cout<<"Ведите количество строк матрицы "<<std::endl;
        in>>mat.rows_count;
        std::cout<<"Введите количество столбцов матрицы "<<std::endl;
        in>>mat.cols_count;
        std::cout<<"Введите элементы матрицы "<<std::endl;

        size_type size = mat.rows_count * mat.cols_count;
        for(size_t i = 0; i < size; ++i)
        {
            value_type dat = 0;
            in >> dat;
            mat.matrix_data.push_back(dat);
        }
    }
