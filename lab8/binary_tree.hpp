...
public:
    ...
    {
        if (!try_insert(data))
            std::cout<<"Error! Element already exist"<<std::endl;
    }
    ...
    {
        if (Root == nullptr)
            std::cout<<"Error! Binary tree not exist"<<std::endl;
        TNode* curNode = Root;
        while (curNode)
        {
            if (curNode->Data == data)
        ...
        std::cout<<"Error! Element does not exist in the tree"<<std::endl;
        return Root;//Если элемента нет в дереве, то вернется корень дерева
    }
